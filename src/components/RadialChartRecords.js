...
function RadialChartRecords({ campaign_id }) {
  ...
  useEffect(() => {
    axios({
      ...
      .then((response) => {
        ...
        let total = 0 ;  
        if(campaign_id){
          totalSent = parseInt(response.data.campaign[0].sent); 
          totalClicked = parseInt(response.data.campaign[0].clicked); 
          totalOpened = parseInt(response.data.campaign[0].open); 
          total = totalSent; 
        }
        else{
  ...
  return (
    <div id="Radialchart" className="w-1/3 rounded-lg shadow-xl mx-10">
...
export default RadialChartRecords;
